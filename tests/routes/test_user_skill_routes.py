...
def test_get_user_skills_route(client, app):
    with app.app_context():
        ...
        response = client.get(f'/api/users/{user.id}/skills', headers=headers)
...
def test_post_user_skills_route(client, app):
    with app.app_context():
        ...
        response = client.post(f'/api/users/{user.id}/skills', json={'skill_id': skill.id}, headers=headers)
...
def test_delete_user_skill_route(client, app):
    with app.app_context():
        ...
        response = client.delete(f'/api/users/{user.id}/skills/{skill.id}', headers=headers)
        assert response.status_code == 200